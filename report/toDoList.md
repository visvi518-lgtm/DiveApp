...
- [x] **NAVER Cloud Platform (Maps)** — 다이빙 로그 위치 선택 지도용, 위 Naver Developers 로그인 키와는 별개 (완료 2026-07-22)
  - [x] NCP 콘솔에서 Maps 애플리케이션 생성, Dynamic Map 활성화
  - [x] Android 패키지명 `com.diveapp.android` 등록
  - [x] 허용 웹 서비스 URL에 배포 도메인(`https://diveapp-web.onrender.com`) 등록
  - [x] NCP Key ID 발급 → Android `local.properties`, Web `.env` + Render 대시보드(`diveapp-web` → Environment)에 모두 반영 완료
...
- [x] `VITE_NAVER_MAP_NCP_KEY_ID` — NAVER Cloud Platform Maps에서 발급받은 NCP Key ID (다이빙 로그 위치 선택 지도용). 로컬 `.env`뿐 아니라 Render 대시보드(`diveapp-web` → Environment)에도 직접 입력 완료, NCP 콘솔의 허용 웹 서비스 URL에 배포 도메인 등록도 완료 (2026-07-22)
...
- [x] `NAVER_MAP_CLIENT_ID` — NAVER Cloud Platform Maps에서 발급받은 NCP Key ID (다이빙 로그 위치 선택 지도용, `NAVER_CLIENT_ID`와는 다른 키). 채워짐, Android 패키지명도 NCP 콘솔에 등록 완료 (2026-07-22) — 단 실기기/에뮬레이터 실행 확인은 아직 미완료
...
- [x] Android 다이빙 위치 지도 연동 UI 구현 완료 (`Docs/12_DiveLogLocation_NaverMap.md`, 2026-07-22), NCP Key ID 발급/입력 완료 — 실기기/에뮬레이터 실행 확인은 아직 미완료
- [x] Web 다이빙 위치 지도 연동 UI 구현 완료 (`Docs/13_DiveLogWeb.md`, 2026-07-22), NCP Key ID 발급/입력 및 Render 배포 환경변수/허용 URL 등록까지 완료
