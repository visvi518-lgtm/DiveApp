...
- Running backlog of unimplemented/unverified items maintained at Report/00_todo_backlog.md (Report 03 Xcode build verification remains the top priority item for iOS, which is paused; live-backend e2e testing, social login SDK integration, and Admin dashboard web screens are now done for Web; Android has live-backend e2e + social login SDK done)

---

## Current Task

User is going through a numbered list in order: 1) live backend e2e testing (DONE, Report 07), 2) real Naver/Google SDK integration (DONE, Report 08 — real account login itself still needs the user's own OAuth app registrations), 3) Admin dashboard web screens (DONE, Report 09), 4) other. Item 4 ("other") is not yet defined. In between, the user asked for email/password login+register on Web (DONE, Report 10) after noticing the login screen only had social buttons — this was an ad-hoc addition, not part of the numbered list. Ask the user what they want next, or offer the remaining backlog items (see Report/00_todo_backlog.md) unless redirected.
